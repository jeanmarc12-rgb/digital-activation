...
// Loader específico para serviços (Barber Studio)
async function loadServicos() {
  const res = await fetch("_data/servicos.json").catch(() => null);
  if (!res || !res.ok) return;
  const data = await res.json().catch(() => null);
  if (!data || !data.items) return;
  const grid = document.getElementById("cms-servicos");
  if (!grid) return;
  grid.innerHTML = data.items.map(s => `
    <div class="servico-card${s.featured ? " featured" : ""}">
      <div class="servico-icon">${s.emoji}</div>
      <h3>${s.name}</h3>
      <p>${s.description}</p>
      <div class="servico-preco">${s.price}</div>
    </div>`).join("");
}
loadServicos();
